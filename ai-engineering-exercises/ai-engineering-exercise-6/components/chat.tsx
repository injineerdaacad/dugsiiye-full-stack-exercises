...
export default function Chat({ conversationId, initialMessages = [], conversationTitle }: ChatProps) {
  ...
  const handleGenerateImage = async (prompt: string) => {
    ...
    try {
      ...
      setMessages([
        ...messages,
        { id: data.userMessageId, role: 'user', parts: [{ type: 'text', text: prompt }] },
        { id: data.assistantMessageId, role: 'assistant', parts: [{ type: 'file', mediaType: 'image/png', url: data.imageDataUrl }] },
      ]);
      router.refresh();
    } catch (err) {
    ...
    <div className="flex flex-col h-full bg-white">
          ...
          {messages.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-500">
                Ask me anything, or ask me to generate an image!
              </p>
            </div>
          )}
      ...
      <div className="bg-white border-t border-rose-100 p-6">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            if (!input.trim() || status !== 'ready' || isGeneratingImage) return;
            if (imageMode) {
        ...
        >
          <div className="flex space-x-3">
            <Button
              ...
              title={imageMode ? 'Switch to text chat' : 'Switch to image generation'}
              className={imageMode
                ? 'px-3 bg-rose-500 hover:bg-rose-600 text-white rounded-xl'
                : 'px-3 border-rose-200 text-rose-600 hover:bg-rose-50 rounded-xl'}
            >
            ...
            <input
              ...
              value={input}
              placeholder={imageMode ? 'Describe the image you want to generate...' : 'Ask a question...'}
