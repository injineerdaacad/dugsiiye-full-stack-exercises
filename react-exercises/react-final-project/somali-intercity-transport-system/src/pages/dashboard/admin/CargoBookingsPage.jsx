...
export default function CargoBookingsPage() {
 ...
 try {
 setIsLoading(true)
 const data = await getCargoBookings()
 setBookings(data || [])
 } catch (error) {
 ...
 try {
 setIsLoading(true)
 const data = await getCargoBookingsByStation(profile.station_id)
 setBookings(data || [])
 } catch (error) {
 console.error('Error loading bookings:', error)
 showToast('Failed to load bookings', 'error')
 ...
 try {
 await updateCargoBooking(bookingId, { status: newStatus })
 ...
 ? 'View all cargo bookings across all stations'
 : 'Manage cargo bookings for your station'}
 </p>
 </div>
 ...
 {booking.status === CARGO_STATUS.PENDING && (
 <>
 <Button
 variant="ghost"
 size="sm"
 ...
 {booking.status === CARGO_STATUS.CONFIRMED && (
 <Button
 variant="ghost"
 size="sm"
 onClick={() =>
